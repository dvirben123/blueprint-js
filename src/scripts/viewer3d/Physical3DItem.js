...
export class Physical3DItem extends Mesh {
  ...
  __itemUpdated(evt) {
    ...
    if (evt.property === "innerRotation") {
      if (this.__loadedItem) {
        this.__loadedItem.rotation.set(
          this.__itemModel.innerRotation.x,
          this.__itemModel.innerRotation.y,
          this.__itemModel.innerRotation.z
        );
      }
      this.__boxhelper.rotation.set(
        this.__itemModel.innerRotation.x,
        this.__itemModel.innerRotation.y,
        this.__itemModel.innerRotation.z
      );
      if (this.__itemStatistics) {
        this.__itemStatistics.rotation.set(
          this.__itemModel.innerRotation.x,
          this.__itemModel.innerRotation.y,
          this.__itemModel.innerRotation.z
        );
  ...
  __gltfLoaded(gltfModel) {
    ...
    this.__loadedItem.traverse((child) => {
      ...
      if (child.material) {
        let materials = child.material.length
          ? child.materials
          : [child.material];
